...
export default function IntelligenceScreen() {
  ...
  const runAnalysis = async (demoModeOverride?: boolean) => {
    setLoading(true);
    const activeDemo = demoModeOverride ?? isDemo;
    try {
      const { data, error } = await supabase.functions.invoke("analyze-disaster-priority", {
        body: { demoMode: activeDemo }
      });
      ...
      showToast(activeDemo ? "Typhoon Odette simulation loaded." : "Live intelligence synchronized.", "success");
    } catch (err) {
      console.error(err);
  ...
  return (
    <Screen 
      ...
      subtitle={isDemo ? "SIMULATION MODE: Analyzing Typhoon Odette landfall impacts." : "AI-driven relief prioritization based on real-time weather and hazard data."}
    >
      <View style={{ gap: 20 }}>
        {/* Scenario Selector */}
        <View style={{ gap: 10 }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
            <Pressable 
              ...
              style={[styles.scenarioPill, !isDemo && styles.scenarioPillActive]}
            >
              <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                <Ionicons name="radio" size={14} color={!isDemo ? "#fff" : theme.colors.text} />
                <Text style={[styles.scenarioText, !isDemo && styles.scenarioTextActive]}>Live Monitoring</Text>
              </View>
            </Pressable>
            <Pressable 
              onPress={() => {
            ...
            >
              <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                <Ionicons name="flask" size={14} color={isDemo ? "#fff" : theme.colors.text} />
                <Text style={[styles.scenarioText, isDemo && styles.scenarioTextActive]}>Demo: Typhoon Odette</Text>
              </View>
            </Pressable>
          </ScrollView>
        </View>
          ...
          {loading ? (
            <View style={styles.mapOverlay}>
              ...
              <Text style={{ marginTop: 12, color: theme.colors.textMuted, fontWeight: "600" }}>
                {isDemo ? "Simulating Odette landfall..." : "Analyzing weather patterns..."}
              </Text>
            </View>
          ) : (
            <WebViewMap 
              ...
              markers={mapMarkers}
              onMarkerPress={(id) => {
                const pin = result?.priorities.find(p => p.cityName === id);
        ...
        {selectedPin && (
          <View style={[styles.card, { borderColor: getPriorityColor(selectedPin.score) }]}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              <View style={{ flex: 1 }}>
                <Text style={styles.label}>STRATEGIC HUD {isDemo ? "(DEMO)" : ""}</Text>
              ...
              onPress={async () => {
                ...
                try {
                  const { data: newEvent, error } = await supabase
                    .from("events")
                    .insert({
                      title: `${isDemo ? "[DEMO] " : ""}Relief: ${selectedPin.cityName} (AI Priority ${selectedPin.score})`,
                  ...
                  showToast(isDemo ? "Demonstration event created." : "Draft event created from AI plan.", "success");
